...
def ManhattanTourist(n_down, n_right, down_matrix, right_matrix):
    ...
    for i in range(1, n_down + 1):
        for j in range(1, n_right + 1):
            s_dict[(i,j)] = max(s_dict[(i-1,j)]+down_matrix[i-1][j], s_dict[(i,j-1)] + right_matrix[i][j-1])
...
if __name__ == "__main__":
    ...
    if stopprogram==0:
        output = ManhattanTourist(n_down, n_right, down_matrix, right_matrix)
